...
页面可选择 `auto`、`bundled` 或 `full` 星表模式。默认缓存目录为
`cache/sky-chart`。

| 模式 | 适用场景 | 代价与行为 |
| --- | --- | --- |
| `auto`（推荐） | 一般使用 | 有已验证完整星表时用 `full`；没有时离线回退到 `bundled` 并显示 `degraded`。 |
| `bundled` | 希望完全离线、快速出图 | 始终使用随包亮星表，背景星较少。 |
| `full` | 需要更高的背景星密度，例如观察 M45 一类疏散星团时 | 必须先下载并校验完整 HYG v4.1 星表；缓存缺失时不会自动联网，渲染会失败。 |

完整星表只影响背景星的密度，不会修复目标名称解析、天气、地平线遮挡或现场可见性。
例如 `M45` 名称解析失败时应改用 RA/Dec 输入或检查目标解析服务；下载完整星表不能解决这类问题。

#### 启用完整星表

只有在你确认需要 `full` 后，才执行下列命令。它会连接固定且已验证的 HYG v4.1
来源，下载当前约 13.6 MB 的压缩数据，并在 `cache/sky-chart/hyg-v4.1/` 写入经
SHA-256 和行数校验的本地缓存。普通启动、`auto`/`bundled` 渲染和导出均不访问该
来源。下载成功后，`full` 渲染读取本地缓存，不会为每次出图重复下载：

```bash
.venv/bin/starskill sky-chart --download-catalog
```

不需要高密度背景星时，保留 `auto` 即可。使用 Agent 运行 `sky-chart` 时，无论用户
是否主动要求 `full`，首次进入星图流程都应先说明上述联网、磁盘缓存和星密度取舍，并
询问你是否愿意下载完整星表；在收到明确回答前不应启动星图流程或触发下载。之后同一
星图流程不必重复询问，除非用户主动改变该选择。
