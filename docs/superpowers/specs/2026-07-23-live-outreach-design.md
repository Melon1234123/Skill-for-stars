...
Node 和 Docker 前置条件执行安装、构建与启动命令，即可运行本机星图、几何计算和
天气建议。仓库不得依赖开发者的绝对路径、既有缓存、私有数据或已安装 Stellarium。
NASA Key、光害快照和 Stellarium 桌面程序是可选增强；缺失时只降级对应面板。
随仓库发布的 `skills/run-starskill` 必须在用户请求本机浏览器体验或全新克隆启动时，
自动执行 README 声明的环境检查、依赖安装、引擎构建、服务启动和健康检查；普通
CLI 天文工作流不得因此无故安装 Node 或构建浏览器。
