...
交付必须支持全新克隆：使用者从空目录克隆仓库后，仅按 README 声明的 Python
前置条件执行安装与启动命令，即可运行本机星图、几何计算和天气建议。仓库不得依赖
开发者的绝对路径、既有缓存、私有数据或已安装 Stellarium。
NASA Key、光害快照和 Stellarium 桌面程序是可选增强；缺失时只降级对应面板。
随仓库发布的 `skills/run-starskill` 必须在用户请求本机星图或全新克隆启动时，
自动执行 README 声明的 Python 环境检查、依赖安装、服务启动和健康检查；普通
CLI 天文工作流不得因此无故启动本地服务。

## 2. 可选 Stellarium 桌面桥接

Stellarium 桌面桥接只访问用户明确开启的 RemoteControl HTTP 服务，默认
仅允许回环地址。它是辅助同步，不是纯 Python 本地星图能够工作的前置条件。

## 3. 架构

```text
Pure Python Local Sky Chart
  |- Astronomy chart renderer
  |- Recommendation view
  `- Local API client
          |
          v
...
本地星图与 MCP 服务共享领域模型、来源记录、缓存策略和建议生成器；它们只在
传输层上不同。纯 Python 本地 API 提供 `/v1`，且硬性绑定 `127.0.0.1`；现有
`starskill-mcp` 保持 `stdio`。二者都不得作为公网服务运行。
...
本地 API 必须硬性绑定回环地址、限制请求体和请求速率，并写入不包含密钥的结构化
日志。纯 Python 星图只能通过受控本地 API 读取结果，不能读取服务器缓存目录或向
Stellarium 桥接传入任意 URL。README 必须给出从克隆、安装 Python 依赖、生成
星图、启动本地服务到配置可选 NASA Key/光害快照的完整步骤。
...
5. 为本地 API 验证回环绑定、速率/请求限制、错误响应和与 MCP 的结果模型一致。
6. 对纯 Python 星图执行固定输入的图像验证：生成结果非空，地点/时间改变后星图和
   建议同步，错误状态可见，文字不重叠。
7. 最终验收运行完整既有测试、新增测试、MCP stdio 发现测试和一次受控的端到端
   本地星图演示；公网提供方只作为单独 live smoke test，不能使离线 CI 不稳定。
